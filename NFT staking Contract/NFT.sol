...
contract Minting is ERC721{
    ...
    constructor() ERC721("Arbitech","ABT"){
        maxSupply = 100 ether;
    }

    mapping(address => uint256) public checkId;
   
    function randomTokenId() public view returns(uint256){
        uint256 randomId;
        // randomId = uint256(keccak256(abi.encodePacked(msg.sender ))) % 100; // only give 1 ids per address
        randomId = uint256(keccak256(abi.encodePacked(block.timestamp,msg.sender ))) % 100; // give different ids per address
        return randomId;
    }
    ...
    function Mint() public  {
        ...
        checkId[msg.sender] = randomId;
        tokenId.push(randomId);
    }
